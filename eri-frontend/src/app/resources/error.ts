 ...
 */
const parseError = (error: Error | EriError | any): string => {
  ...
  const errorMap: Record<string, string> = {
    ...
    username_must_be_at_least_3_letters:
  ...
  return (
    message.replace("execution reverted:", "").replace("error:", "").trim() ||
    "An unknown error occurred"
  );
};

function addressZero(): string {
